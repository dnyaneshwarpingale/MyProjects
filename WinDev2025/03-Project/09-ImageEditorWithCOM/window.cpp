...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpszCmdLine, int iCmdShow)// process chya ayshyachi survat 
{
    ...
    if (FAILED(hr))
    {
        MessageBox(NULL, TEXT("COM Initialization failed"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
    ...
    hwnd = CreateWindow(szAppName,                   // name of your window class
                        TEXT("Image Editor and Color Picker"), // window caption bar text
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;    
    ...
    case WM_CREATE:
        ...
        hr = CoCreateInstance(CLSID_DesaturationSepia, NULL, CLSCTX_INPROC_SERVER, IID_IDesaturation, (void**)&pIDesaturation);
        if (FAILED(hr))
        {
            MessageBox(NULL, TEXT("IDesaturation Interface could not obtained"), TEXT("COM Error"), MB_OK | MB_ICONERROR);
            DestroyWindow(hwnd);
        }
    ...
    case WM_PAINT:
        ...
        if (hBitmap) // if a bitmap is loaded
        {
            ...
            else if (imageEffect == 1)
            {
                for (yRow = 0; yRow < resizedWindowHeight; yRow++) 
                {
                        ...
                        for (xColumn = 0; xColumn < resizedWindowWidth; xColumn++) 
                        {
                            ...
                            //Apply Desaturation effect using pIDesaturation interface
                            COLORREF desaturationColor;
                            pIDesaturation->ApplyDesaturation(originalPixelColor, &desaturationColor); 

                            // set final dsaturated color back to the same pixel or at coordinate xColumn, yRow
                          
                            SetPixel(hdc, xColumn, yRow, desaturationColor);//this will here in wmpaint.  
            ...
            else if (imageEffect == 2)
            {
                ...
                if (FAILED(hr))
                {
                    MessageBox(NULL, TEXT("ISepia interface not obtained"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
                    DestroyWindow(hwnd);
                }
                ...
                for (yRow = 0; yRow < resizedWindowHeight; yRow++) 
                {
                        ...
                        for (xColumn = 0; xColumn < resizedWindowWidth; xColumn++)
                        {
                            ...
                            // from original pixel color obtained in above step seperate out the Red Green and Blue components 
                            // using them calculate Sephiya SR SG and S
                            COLORREF sepiaColor;
                            pISepia->ApplySepia(originalPixelColor, &sepiaColor);
            ...
            {
                hr = pIDesaturation->QueryInterface(IID_INegative, (void**)&pINegative);
                if (FAILED(hr))
                {
                    MessageBox(NULL, TEXT("INegative interface not obtained"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
                    DestroyWindow(hwnd); 
                }
                ...
                for (yRow = 0; yRow < resizedWindowHeight; yRow++) 
                {
                        ...
                        for (xColumn = 0; xColumn < resizedWindowWidth; xColumn++) 
                        {
                            ...
                            COLORREF negativeColor;
                            pINegative->ApplyNegative(originalPixelColor, &negativeColor);
                            // set final dsaturated color back to the same pixel or at coordinate xColumn, yRow
                           
                            SetPixel(hdc, xColumn, yRow, negativeColor); 
...
INT_PTR CALLBACK MyDlgProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        {
            EnableWindow(GetDlgItem(hDlg, IDD_REGISTER_DIALOG), TRUE); 
        }
    ...
    case WM_CTLCOLORDLG:
        hDlgBrush = CreateSolidBrush(RGB(182, 228, 251)); //candy colors (212, 173, 207) ,(203,238,243)
...
INT_PTR CALLBACK AboutDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        hIcon = LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(DHP_ICON));  
        if (hIcon != NULL)
        {
            SendDlgItemMessage(hDlg, DHP_ICON, STM_SETICON, (WPARAM)hIcon, 0); 
...
void CreateNormalizedColorPickerLogFile(const char* firstName, const char* surname)
{
    SYSTEMTIME st; // declare the systemtime variable 
    ...
    char normalizecolorPickerlogFileName[100];
    char timestamp[20]; // Buffer to hold timestamp
    ...
    wsprintf(normalizecolorPickerlogFileName, "%s%s-NormalizedColorPicker-%s.txt", firstName, surname, timestamp); 

    // open the log file for writing 
    gpFile_ColorPickerNormalizedLog = fopen(normalizecolorPickerlogFileName, "w");
    if (gpFile_ColorPickerNormalizedLog == NULL)
    {
        MessageBox(NULL, "color picker Log file not created", "Userlog Error", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    // Log Common Details
    fprintf(gpFile_ColorPickerNormalizedLog, "Register User Name:%s %s\n", firstName, surname);
    fprintf(gpFile_ColorPickerNormalizedLog, "User Registration Date: %02d/%02d/%04d\n", st.wDay, st.wMonth, st.wYear);
    fprintf(gpFile_ColorPickerNormalizedLog, "User Registration Date: %02d:%02d:%02d\n", st.wHour, st.wMinute, st.wSecond);
}
