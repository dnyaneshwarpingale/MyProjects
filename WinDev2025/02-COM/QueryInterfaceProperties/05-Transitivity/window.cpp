...
#include "CombinedContainmentHeaderFile.h"
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
       ...
       //Proving transitivity property of queryInterface.
       hr = pIDevision->QueryInterface(IID_ISum, (void**)&pISumAgain);
       if (FAILED(hr))
       {
           MessageBox(NULL, TEXT("cocreate instance failed to obtain pISumAgain from IDevision"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
           DestroyWindow(hwnd);
       }

       if (pISum = pISumAgain) {
           MessageBox(NULL, TEXT("Transitivity Proved"), TEXT("Property5"), MB_OK );
           DestroyWindow(hwnd);
       }
       else {
           MessageBox(NULL, TEXT("Transitivity not  Proved"), TEXT("Property5"), MB_OK);
           DestroyWindow(hwnd);
       }
        DestroyWindow(hwnd); 
        break;
