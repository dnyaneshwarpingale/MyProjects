...
#include "CombinedHeaderFileForAgreegationClient.h"
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        //Aggregation subflow no.1 in end client calling cocreateinstance to obtain outer interface.
        ...
        //subflow no:2 calling queryinterface on outer interface pointer  and to obtain another interface.


       hr = pISum->QueryInterface(IID_ISubtract ,(void **) & pISubtract);
       if (FAILED(hr))
       {
           MessageBox(NULL, TEXT("cocreate instance failed to obtain ISubtract interface upon isum "), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
           DestroyWindow(hwnd); 
       }
      ...
      pISubtract->SubtractionOfTwoIntegers(num1, num2, &sub); 
       ...
       //subflow no :3 pisubtract varti queryinterface call karun inner cha interface milvan 
             
       hr = pISubtract->QueryInterface(IID_IMultiplication, (void**)&pIMultiplication); 
       if (FAILED(hr))
       {
           MessageBox(NULL, TEXT("cocreate instance failed to obtain IMultiplication interface"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR); 
           DestroyWindow(hwnd); 
       }

       pIMultiplication->MultiplicationOfTwoIntegers(num1, num2, &mult); 
       wsprintf(str, TEXT("Multiplication of %d and %d : %d"), num1, num2, mult);  
       MessageBox(NULL, str, TEXT("Message"), MB_OK);  
        
       //subflowno:4  inner interface varti dusra inner interface milvane.
       
       hr = pIMultiplication->QueryInterface(IID_IDevision, (void**)&pIDevision);  
       if (FAILED(hr)) 
       {
           MessageBox(NULL, TEXT("cocreate instance failed to obtain IDevision interface"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
           DestroyWindow(hwnd);
       }

       pIDevision->DevisionOfTwoIntegers(num1, num2, &dev); 
       wsprintf(str, TEXT("Devision of %d and %d : %d"), num1, num2, dev);  
       MessageBox(NULL, str, TEXT("Message"), MB_OK); 


       //Subflow no.: 5 inner interface varti queryinterface karun outer interface milavane.

       hr= pIDevision->QueryInterface(IID_ISum, (void**)&pISumAgain);
       if (FAILED(hr)) 
       {
           MessageBox(NULL, TEXT("cocreate instance failed to obtain Outer interface from inner"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR); 
           DestroyWindow(hwnd); 
       }

       MessageBox(NULL, TEXT("Successfully obtained outer interface from inner"), TEXT("yeah"), MB_OK );

       //subFlow 6 : outer interface varti query interface karun iunknown milvane
      
      hr = pISum->QueryInterface(IID_IUnknown, (void**)&pIUnknownFromISum); 
      if (FAILED(hr))
      {
          MessageBox(NULL, TEXT("cocreate instance failed to obtain Iunknown interface from Outer"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
          DestroyWindow(hwnd); 
      }

      //subflow 7: inner interface varti query interface karun iunknown milvane and check whether it is same or different.

      hr = pIMultiplication->QueryInterface(IID_IUnknown, (void**)&pIUnknownFromIMultiplication);
      if (FAILED(hr))
      {
          MessageBox(NULL, TEXT("cocreate instance failed to obtain Iunknown interface from Inner"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
          DestroyWindow(hwnd);
      }
      ...
      {
          MessageBox(NULL, TEXT("Both are same so Identity proved"), TEXT("Yeah"), MB_OK ); 
          DestroyWindow(hwnd); 
      }
...
void SafeInterfaceRelease(void)
{
    ...
    if (pIUnknownFromIMultiplication)
    {
