// Win32 headers
#include <windows.h>

#include "Window.h"

#include "AutomationServerHeaderForCClient.h"

// Global function declarations
LRESULT CALLBACK WndProc(HWND, UINT, WPARAM, LPARAM);

//Global declaration of interface pointers.
IDispatch* pIDispatch = NULL;


// Entry-point function
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)// process chya ayushyachi survat
{
    // Variable declarations
    WNDCLASSEX wndClass;
    HWND hwnd;
    MSG msg;
    TCHAR szAppName[] = TEXT("DHP_Window");
    HRESULT hr = S_OK;

    // Code
    //initializing com engine.
    hr = CoInitialize(NULL);// it is from ole32.lib 

    if (FAILED(hr))
    {
        MessageBox(NULL, TEXT("co Initilized failed"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
        exit(0);

    }

    memset((void*)&wndClass, 0, sizeof(WNDCLASSEX));
    // Window class initialization
    wndClass.cbSize = sizeof(WNDCLASSEX);
    wndClass.style = CS_HREDRAW | CS_VREDRAW;
    wndClass.cbClsExtra = 0;
    wndClass.cbWndExtra = 0;
    wndClass.lpfnWndProc = WndProc;
    wndClass.hInstance = hInstance;
    wndClass.hbrBackground = (HBRUSH)GetStockObject(WHITE_BRUSH);
    wndClass.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(DHP_ICON));
    wndClass.hCursor = LoadCursor(NULL, IDC_ARROW);
    wndClass.lpszClassName = szAppName;
    wndClass.lpszMenuName = NULL;
    wndClass.hIconSm = LoadIcon(hInstance, MAKEINTRESOURCE(DHP_ICON));

    // Registration of window class
    RegisterClassEx(&wndClass);
    // Create window
    hwnd = CreateWindow(szAppName,                   // name of your window class
        TEXT("DHP:My First Window"), // window caption bar text
        WS_OVERLAPPEDWINDOW,         // window style overlappedwindow
        //  it is combination of 6 window styles ws_overlapped | ws_caption |
//  ws_thickframe | ws_sysmenu | ws_minimizebox | ws_maximizebox
CW_USEDEFAULT, // x from desktop origin
CW_USEDEFAULT, // create window y from desktop origin
CW_USEDEFAULT, // width
CW_USEDEFAULT, // height
NULL,          // parent window here is desktop window is parent or HWND_DESKTOP
NULL,          //  HMENU HANDLE OF MENU
hInstance,     // handle to instance
NULL);         //
    // create window creates the window only in only in memory
// Show window
    ShowWindow(hwnd, iCmdShow);

    // Paint background of window
    UpdateWindow(hwnd);

    // Message loop
    while (GetMessage(&msg, NULL, 0, 0)) // second parameter -> handle to the window ... null--> because default msges and child window msges
    {                                    // parameter 3 and 4--> min and max limit of msges
        TranslateMessage(&msg);          // simplify the msg
        DispatchMessage(&msg);
    }

    //Uninitializing the COM engine.
    CoUninitialize();//=>COfreeunusedlibraries()=>lpfn2=>DllCanUnloadNow()=>CoFreeLibrary()=>Freelibrary().


    return ((int)msg.wParam);
}

// Callback function
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    // Code
    // Function prototype
    void SafeInterfaceRelease(void);

    //varible declaration
    TCHAR str[255];
    unsigned int x = 0;
    unsigned int y = 0;
    HRESULT hr = S_OK;
    int num1 = 23;
    int num2 = 30;

    int sum = 0;
    int sub = 0;

    //IDispatch client related variables
    //exported functionality token names. whoes dispatchids we must obtain to invoke this functionaity.
    OLECHAR* szFunctionName1 = L"SumOfTwoIntegers"; //literal(L) sttring and it is an Unicode string.
    OLECHAR* szFunctionName2 = L"SubtractionOfTwoIntegers";

    DISPID dispid; //varible to store dispatch id of the functionality invoked.

    //variables to send parameters to the invoke functionlity
    DISPPARAMS params; 
   static VARIANT vArgs[2];

   static VARIANT vRetval; //varibale to hold actual return value of the invoke functionalities.


    switch (iMsg)
    {
    case WM_CREATE:
        hr = CoCreateInstance(&CLSID_MyMath, NULL, CLSCTX_INPROC_SERVER, &IID_IDispatch, (void**)&pIDispatch); //CLSCTX_INPROC_SERVER==which type of server 

        if (FAILED(hr))
        {
            MessageBox(NULL, TEXT("cocreate instance failed to obtain IDispatch interface"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
            DestroyWindow(hwnd); 
        }

        
        //prepare the parameters to be sent to the invoked functionality.
        VariantInit(vArgs);//initaiizing variant variables

        vArgs[0].vt = VT_INT;
        vArgs[0].intVal = num2;

        vArgs[1].vt = VT_INT;
        vArgs[1].intVal = num1; 

        params.rgvarg = vArgs;//registered variant arguments.
        params.cArgs = 2; //count of arguments 
        params.rgdispidNamedArgs = NULL;
        params.cNamedArgs = 0;

        ///prepared tthe return value  variant varibale which will take return value of invoked functionalities.
        VariantInit(&vRetval);

        //Get Dispatch ID of SumOFTwoIntegers() functionlity.
        hr = pIDispatch->lpVtbl->GetIDsOfNames(pIDispatch, &IID_NULL, &szFunctionName1, 1, GetUserDefaultLCID(), &dispid);

        if (FAILED(hr)) 
        {
            MessageBox(NULL, TEXT("failed to obtain Sumoftwointegers dispatchid "), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
            DestroyWindow(hwnd); 
        }

        //using above obtained dispid call inovoke to invoke sumoftwointegers functionalities
        hr = pIDispatch->lpVtbl->Invoke(pIDispatch,dispid, &IID_NULL, GetUserDefaultLCID(), DISPATCH_METHOD, &params, &vRetval, NULL, NULL);

        if (FAILED(hr))
        {
            MessageBox(NULL, TEXT("failed to INVOKE Sumoftwointegers FUNCTIONALITY "), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
            DestroyWindow(hwnd); 
        }

        //Obtain the return value of invoke functionality.
        sum = vRetval.lVal;
        wsprintf(str, TEXT("Sum of %d and %d : %d"),num1,num2, sum);  
        MessageBox(NULL, str, TEXT("Message"), MB_OK);  


        //Get Dispatch ID of SubtractionOFTwoIntegers() functionlity.
        hr = pIDispatch->lpVtbl->GetIDsOfNames(pIDispatch, &IID_NULL, &szFunctionName2, 1, GetUserDefaultLCID(), &dispid);

        if (FAILED(hr)) 
        {
            MessageBox(NULL, TEXT("failed to obtain SubtractionOFTwoIntegers() dispatchid "), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
            DestroyWindow(hwnd); 
        }

        //using above obtained dispid call inovoke to invoke SubtractionOFTwoIntegers() functionalities

        hr = pIDispatch->lpVtbl->Invoke(pIDispatch,dispid, &IID_NULL, GetUserDefaultLCID(), DISPATCH_METHOD, &params, &vRetval, NULL, NULL);

        if (FAILED(hr))
        {
            MessageBox(NULL, TEXT("failed to INVOKE SubtractionOFTwoIntegers() FUNCTIONALITY "), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
            DestroyWindow(hwnd);
        }
     
        //Obtain the return value of invoke functionality.
        sub = vRetval.lVal; 
      
       wsprintf(str, TEXT("Subtraction of %d and %d : %d"), num1, num2, sub);
       MessageBox(NULL, str, TEXT("Message"), MB_OK);  


        DestroyWindow(hwnd); 
        break;

    case WM_KEYDOWN:
        switch (wParam) {
        case VK_ESCAPE:
            DestroyWindow(hwnd);
            break;
        }

        break;

    case WM_DESTROY:
        MessageBox(NULL, TEXT("WM_DESTROY arrived"), TEXT("Message"), MB_OK);
        VariantClear(&vRetval);
        VariantClear(vArgs); 


        SafeInterfaceRelease();
        PostQuitMessage(0); // parameter of postquitmsg 0 is wparam of wm_quit
        break;              // from this above 3line are  the message handler of wm_destroy

    default:
        break;
    }

    return (DefWindowProc(hwnd, iMsg, wParam, lParam)); // default window procedure .. operating system window procedure
} // it send all handled and unhandled messages to os

//Userdefined function for safely releasing the interface pointer.
void SafeInterfaceRelease(void)
{
    //code
    if (pIDispatch)
    {
        pIDispatch->lpVtbl->Release(pIDispatch);
        pIDispatch = NULL;
    }


}

